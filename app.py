...
def load_artifacts():
    ...
    return feature_names, unique_categorical_values, combined_location_mapping

model = load_model()
feature_names, unique_categorical_values, combined_location_mapping = load_artifacts()
...
def on_zip_code_change():
    ...
    # No further resets needed as zip code is the last in the hierarchy for filtering
...
if 'selected_neighborhood_name' not in st.session_state:
    # Get valid neighborhood for initial borough
...
if 'selected_zip_code' not in st.session_state:
    # Get valid zip code for initial borough and neighborhood
...
# Latitude and Longitude inputs for map visualization
st.subheader('Property Location (for Map Visualization)')
latitude = st.number_input('LATITUDE', min_value=-90.0, max_value=90.0, value=st.session_state.selected_latitude, format="%.6f", help='Latitude coordinate of the property')
longitude = st.number_input('LONGITUDE', min_value=-180.0, max_value=180.0, value=st.session_state.selected_longitude, format="%.6f", help='Longitude coordinate of the property')

# Store current lat/lon in session state to maintain values across reruns
...
folium.Marker([latitude, longitude], popup='Property Location').add_to(m)
